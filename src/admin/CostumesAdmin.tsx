  ...
  return (
    <div className="admin-card">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        ...
        <button onClick={() => nav("/admin/costumes/new")} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <span>+</span> Добавить
        </button>
      </div>
        ...
        <div style={{ display: 'grid', gap: '12px' }}>
          {list.map(c => (
            <div key={c._id} className="admin-item">
              ...
              <div>
                <button onClick={() => nav(`/admin/costumes/${c._id}`)}>Редактировать</button>
                <button className="danger" onClick={() => remove(c._id)}>Удалить</button>
              </div>
            </div>
          ))}
        </div>
